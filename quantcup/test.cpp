...
int test_cancel(t_order orders1[],
                unsigned orders1_len,
                t_orderid cancels[],
                unsigned cancels_len,
                t_order orders2[],
                unsigned orders2_len,
                t_execution execs[],
                unsigned execs_len);
...
#define TEST_CANCEL(num, orders1, cancels, orders2, execs)                                         \
    ...
    t_execution x##num[] = execs;                                                                  \
    correct += test_cancel(o1st##num, sizeof(o1st##num) / sizeof(t_order), c##num,                 \
                           sizeof(c##num) / sizeof(t_orderid), o2nd##num,                          \
                           sizeof(o2nd##num) / sizeof(t_order), x##num,                            \
                           sizeof(x##num) / sizeof(t_execution));
#define X ,
...
int main()
{
    ...
    TEST(9, {oa101x100 X ob101x25 X ob101x25 X ob101x25 X ob101x25 X ob101x25},
         {xa101x25 X xb101x25 X xa101x25 X xb101x25 X xa101x25 X xb101x25 X xa101x25 X xb101x25});
    ...
    TEST_CANCEL(14, {ob101x100 X ob101x25x X ob101x25x X ob101x50}, {1 X 4 X 3}, {oa101x50},
                {xb101x25x X xa101x25});
...
int exec_eq(t_execution* e1, t_execution* e2)
{
    ...
    {
        if (e1->symbol[i] == '\0' && e2->symbol[i] == '\0')
            break;
        eq = eq && e1->symbol[i] == e2->symbol[i] && e1->symbol[i] == e2->symbol[i];
    }
    eq = eq && e1->side == e2->side && e1->price == e2->price && e1->size == e2->size;
    return eq;
}
...
int assert_execs(t_execution execs[], unsigned execs_len)
{
    ...
    {
        if (!((exec_eq(&execs[i], &execs_out[i]) && exec_eq(&execs[i + 1], &execs_out[i + 1])) ||
              (exec_eq(&execs[i], &execs_out[i + 1]) && exec_eq(&execs[i + 1], &execs_out[i]))))
        {
            printf("executions #%u and #%u,\n"
                   ...
                   "{symbol=%s, trader=%s, side=%i, price=%ld, quantity=%ld}.\n"
                   "Stopped there.\n",
                   i, i + 1, execs_out[i].symbol, execs_out[i].trader, execs_out[i].side,
                   execs_out[i].price, execs_out[i].size, execs_out[i + 1].symbol,
                   execs_out[i + 1].trader, execs_out[i + 1].side, execs_out[i + 1].price,
                   execs_out[i + 1].size, execs[i].symbol, execs[i].trader, execs[i].side,
                   execs[i].price, execs[i].size, execs[i + 1].symbol, execs[i + 1].trader,
                   execs[i + 1].side, execs[i + 1].price, execs[i + 1].size);
            return 0;
        }
    }
    return 1;
